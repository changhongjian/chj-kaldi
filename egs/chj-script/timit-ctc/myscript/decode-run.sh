...
#	data=$data/decode_method_v1
	...
	myscript/ark_vec-chg48_39.rb "$data/output.maxid.ark" >"$data/output_39_min.ark" 2>>$log || exit 1;
	myscript/ark_vec-chg48_39.rb "$labels" >"$data/target_39_min.ark" 2>>$log || exit 1;
	chj-decode-ctc-by-shrink-id "ark:$data/output_39_min.ark" "ark:$data/target_39_min.ark" "$dir/result.txt-notuse" 2>>$log || exit 1;

	myscript/ark_vec-ctc-shrink.rb "$labels" > "$data/target_48_min.ark" 2>>$log || exit 1; #仅是为了去掉0
	chj-decode-ctc-by-shrink-id "ark:$data/output.maxid.ark" "ark:$data/target_48_min.ark" "$dir/result.txt-notuse" 2>>$log || exit 1;

#chj-decode-ctc-by-shrink is equal to computer-wer
	compute-wer --text --mode=present "ark:$data/target_48_min.ark" "ark:$data/output.maxid.ark"  - | grep "WER" >"$dir/result48_1.txt"  2>>$log || exit 1;
	compute-wer --text --mode=present "ark:$data/target_39_min.ark" "ark:$data/output_39_min.ark" - | grep "WER" >"$dir/result39_1.txt" 2>>$log || exit 1;
