...
train_tool="chj-ctc-lstm-stream2"  #训练的程序

#-------------与程序有关的
runname="ctc-d1-lr0.001hf0.3-cuda-s1_8-phone39"
...
if [ $stage -le $cnt ]; then
	...
	test_labels="$mfcc39_index/targets39/test.$traintype.ark"
	$myscript/decode-run-simple.sh --nj 20 --cmd "$decode_cmd" --nnet_forward "nnet-lstm-forward" --targets_delay $targets_delay --acwt 0.2 \
    --feature_transform $feature_transform --nnet $dir/final.nnet  \
	--class_frame_counts $num_tgt --decode_tool "chj-decode-ctc-by-shrink-id"   \
    $test_feats $test_labels $dir/decode_test || exit 1;
	
	dev_feats="$mfcc39_index/dev/feats.scp"
    dev_labels="$mfcc39_index/targets39/dev.$traintype.ark"
    $myscript/decode-run-simple.sh --nj 20 --cmd "$decode_cmd" --nnet_forward "nnet-lstm-forward" --targets_delay $targets_delay --acwt 0.2 \
    --feature_transform $feature_transform --nnet $dir/final.nnet  \
    --class_frame_counts $num_tgt --decode_tool "chj-decode-ctc-by-shrink-id"   \
