...
nnet_forward="nnet_forward"  #长虹剑自己添加
targets_delay=0
...
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    $nnet_forward --feature-transform=$feature_transform --targets-delay=$targets_delay  --no-softmax=true --class-frame-counts=$class_frame_counts --use-gpu=$use_gpu $nnet "$feats" ark:- \| \
