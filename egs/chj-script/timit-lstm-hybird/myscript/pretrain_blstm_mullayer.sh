...
cp  $parentdir/final.mdl $dir/final.mdl
...
for depth in $(seq 1 $nn_depth); do
	...
	if [ "$depth" == "1" ];then
		...
		lstm_layers=1
		$script_pos/make_blstm_proto.py $proto_opts  \
	...
	if [ -f $dir/$depth.nnet ]; then
		...
		cp  $parentdir/final.mdl $dir/final.mdl  #又复制了一下
