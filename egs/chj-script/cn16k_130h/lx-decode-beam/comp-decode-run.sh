...
if [ $stage -le 2 ]; then
  beams=(10 20 30 50 300) 
  names=(beam-search-withoutlm beam-search-with-kenlm beam-search-with-fst)
...
#		continue
      ...
      $name --apply_log=true --beam=$beam_num  "ark:$data/output.ark" $lmfile $outark 2>$log || exit 1;
