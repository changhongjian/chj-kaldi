...
class_frame_counts= # non-default location of PDF counts (optional)   没用了
...
if [ $stage -le 1 ]; then
	...
	compute-wer --text --mode=present "ark:$data/target_min.ark" "ark:$data/bestpath.ark"  - | grep "WER" >"$dir/result.txt"  2>>$log || exit 1;
