...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    bool do_once=false;
	po.Register("do_once", &do_once, "做一次就停止");
    ...
    if (po.NumArgs() != 3) {
	  fprintf(stderr,"your numargs is %d",po.NumArgs());
      po.PrintUsage();
      exit(1);
    }
  ...
  // only useful for LSTM
    int32 num_stream = 1; 
    std::vector<int> new_utt_flags(num_stream, 1);
    // nstream is 1 for decoding.
    ...
    double time_now = 0;
    int32 num_done = 0;
    Matrix<BaseFloat> mat_shift;
    int32 feat_dim, frm_num;
    int32 curt = 0;
    int32 lent = 0;
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
        ...
        frm_num = mat.NumRows();
        feat_dim = mat.NumCols();
        mat_shift.Resize(frm_num, feat_dim); 

        curt = 0;
        lent = frm_num;
		for (int32 t = 0; t < frm_num; ++t){
			if (curt + targets_delay < lent){
				mat_shift.Row(t).CopyFromVec(mat.Row(curt+targets_delay));
			} else {
				mat_shift.Row(t).CopyFromVec(mat.Row(lent-1));
			}
			curt++;
		}
		...
		if(do_once)break;//我只是想试试只得到特征数目这样行不行
