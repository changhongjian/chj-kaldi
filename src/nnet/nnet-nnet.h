...
///自己加的
  void Reset(std::vector<int> &stream_reset_flag){
      int32 L = 0;
 //kaldi::nnet1::LstmProjectedStreams *comp=dynamic_cast<LstmProjectedStreams*>(components_[L]);
      for(L++; L<=NumComponents()-2; L++) {
            if(components_[L]->GetType()==Component::myLstmProjectedStreams){
                LstmProjectedStreams *comp=(kaldi::nnet1::LstmProjectedStreams*)components_[L];
                comp->Reset(stream_reset_flag);
            }else if(components_[L]->GetType()==Component::myBLstmProjectedStreams){
                BLstmProjectedStreams *comp=(kaldi::nnet1::BLstmProjectedStreams*)components_[L];
                comp->Reset(stream_reset_flag);
            }
      }
  }
