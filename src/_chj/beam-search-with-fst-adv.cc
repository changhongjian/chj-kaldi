...
float LM_log_prob(const StdFst & fst,const  int &  k ,const std::vector<int>& path,float & lm_wght,int &  order){
...
std::vector<int32> BeamSearch(const StdFst & lm_mdl, const Matrix<BaseFloat>& nnet_out, int32 & beam, float & lm_wght, int & order)
{
...
//cerr<<kLogZeroBaseFloat<<" "<<kLogZeroFloat<<endl;
  ...
  try {
    ...
    // Load ARPA LM
    StdFst *  pfst = StdFst::Read(arpa_lmfile.c_str());
    StdFst &  lm_mdl=*pfst;
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
     ...
     static float blank_prob_min=Log(0.9);
     //cout<<blank_prob_min<<"---"<<endl;
     int cols=nnet_out_host.NumCols() , rows=nnet_out_host.NumRows();
	 int rcnt=0;
	 for(int i=0;i<rows;i++){
		if(nnet_out_host(i,0)>blank_prob_min){
			//cout<<i+1<<" / "<<nnet_out_host(i,0)<<" ";
			//cout<<i+1<<" ";
			rcnt++;
		}
	//		for(int j=0;j<cols;j++){
	//			cout<<nnet_out_host(i,j)<<" ";
	//		}
	 }
	 static int cnt=0;
	 cnt++;
     cout<<endl<<"finished "<<cnt<<" / "<<rows<<" / "<<rcnt<<" / "<<(rcnt*1.0/rows)<<endl;
     continue;
