...
int main(int argc, char *argv[]) {
  ...
  chj_logname="debug-bestpath.log";
  ...
  try {
    ...
    KALDI_ASSERT(beam > 0);
    int n=1;
	std::string feature_rspecifier=po.GetArg(n++);
    std::string chars_listfile;
    if(po.NumArgs()==4){
        chars_listfile= po.GetArg(n++);
    }
    std::string arpa_lmfile = po.GetArg(n++);
    std::string result_wspecifier = po.GetArg(n++);
