...
const Arc * CHJ_Find_for_lm(size_t olabel,bool findend=false){
//这个只符合我的需求同时，还要保证已经排好序
    int num=data_.narcs;
    const Arc *arcs=data_.arcs;
    int bg=0,ed=num-1,mid;
	if(findend){
		if(arcs[ed].olabel==olabel) return arcs+ed;
    }else if(arcs[ed].olabel==0) ed--;
    while(bg<=ed){
        mid=(bg+ed)/2;
        if( arcs[mid].olabel==olabel  ){
            return arcs+mid;
        }else if( arcs[mid].olabel<olabel  ){
            bg=mid+1;
        }else{
            ed=mid-1;
        }
